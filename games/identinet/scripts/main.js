...
"use strict";
{
	...
	const HANDLER_CLASS = class ppstudio_newgroundsio_pluginDOMHandler extends globalThis.DOMElementHandler
	{
                ...
                (status)=>{
                    ...
                    switch (status) {
                        ...
                        case NGIO.STATUS_LOGIN_SUCCESSFUL:{
                            ...
                            if (NGIO.user.id!==null){
                                this.PostToRuntime("update-user-data",{"user": this.getUserData(),"session":this.getSession()});
                                this.#justLoggedin=true;
                            }
                            else {
                                this._resumeRuntime();
                                this.PostToRuntime("on-login-success",{"user": this.getUserData(),"session":this.getSession()});
                            }
                        } break;
                        ...
                        case NGIO.STATUS_READY:
                            this._updatePreloadedItems().then(
                                ()=>{
                                    if (!this.isReady){
                                        this.isReady=true;
                                        setTimeout(()=>{this.PostToRuntime("on-ready",{"session":this.getSession()});},1000);
                                    }
                                    ...
                                    if (this.#justLoggedOut){
                                        ...
                                        this.PostToRuntimeAsync("update-user-data",{ "user": this.getUserData(),"session":this.getSession()})
                                            .then(
                                                ()=>{
        ...
        getSession(){
            ...
            return { //Serializing data
                "id": NGIO.session.id,
                "expired":NGIO.session.expired,
                "passport_url":NGIO.session.passport_url,
                "remember":NGIO.session.remember
            }
        }
