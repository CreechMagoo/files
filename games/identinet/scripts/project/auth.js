import { get, post, del, setAuthToken, clearAuthToken, } from "./base/api-client.js";
/**
 * authenticate with newgrounds using session id
 * call this after the player is assigned a session from ngio
 * @param sessionId - session id
 */
export async function authenticateNewgrounds(sessionId) {
    const response = await post("/auth/newgrounds/authenticate", {
 ...
 */
export async function getItchOAuthUrl() {
    return get("/auth/itchio/authorization-url");
}
/**
 ...
 */
export async function handleOAuthPopup(authUrl, pollId, windowName = "oauth_login", timeoutMs = 5 * 60 * 1000) {
    return new Promise((resolve) => {
        ...
        try {
            popup = window.open(authUrl, windowName, "width=500,height=600,scrollbars=yes,resizable=yes,location=yes");
 ...
 */
export async function loginWithItch() {
    const maxRetries = 3;
