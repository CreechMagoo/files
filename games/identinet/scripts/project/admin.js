 ...
 */
export async function listAllCharacters(page = 1, limit = 50, showDeleted = false) {
 ...
 */
export async function deleteCharacter(characterId, reason) {
    ...
    if (response.success) {
        ...
        if (characterId === RUNTIME.globalVars.SERVER_uploadedCharacterID)
 ...
 */
export async function getRecentCharacters(limit = 20) {
 ...
 */
export async function getDeletedCharacters(limit = 50) {
    const response = await listAllCharacters(1, limit, true);
    if (!response.success || !response.data) {
        return response;
    }
    const deletedCharacters = response.data.characters.filter((char) => char.is_deleted);
    return {
        success: true,
        data: {
...
export async function getAdminStats() {
    try {
        const [usersResponse, charactersResponse, deletedResponse] = await Promise.all([
            listAllUsers(1, 1000),
