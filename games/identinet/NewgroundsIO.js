class PopupWrapper{
	static openModal(headerText, descriptionText) {
        ...
        style.innerHTML = `
            ...
            .modal-content {
                display: flex; /* Usamos flexbox para mostrar la imagen y el texto lado a lado */
        ...
        modalImage.src = "https://img.ngfiles.com/wiki/uploads/968000/iu_968880_1.gif";
...
class NGIO
{
    ...
    static ngpopupwindow = null; //TristanMX ENH
    static get ngWindow(){return NGIO.ngpopupwindow} //TristanMX ENH
	...
	static init(appID, aesKey, options)
	{
		if (!this.isInitialized) {
			...
			this.#ngioCore.addEventListener("serverResponse", function(e) {
				if (options["onServerResponse"]) //TristanMX ENH
					if (typeof options["onServerResponse"]=='function') //TristanMX ENH
						options["onServerResponse"](e); //TristanMX ENH
	 ...
	 */
	static openLoginPage(type)
	{
		if (!this.#loginPageOpen) {
			this.#skipLogin = false;
			this.#sessionReady = false;
			this.#loginPageOpen = true;
			this.session.openLoginPage(type);
		} else {
	...
	static getScores(boardID, options, callback, thisArg)
	{
		...
		board.getScores(_options, function() {
			let _s=[];
			if (this.lastGetScoresResult!==null)
				_s=this.lastGetScoresResult.scores; //TristanMX FIX

			if (typeof(callback) === "function") thisArg ? callback.call(thisArg, board, _s, _options) : callback(board, _s, _options);
		}, this);
	...
	{
		this.ngioCore.executeComponent(this.ngioCore.getComponent('Event.logEvent', {event_name:eventName}), function(d) { //d:TristanMX fix
			if (typeof(callback) === 'function') thisArg ? callback(thisArg, this.lastLoggedEvent,d) : callback(this.lastLoggedEvent,d);
		}, this);
	}
	...
	static #checkLocalVersion(callback, thisArg) {
		...
		this.ngioCore.executeQueue(function() {
			...
			if (!this.#legalHost) {
				console.warn("NGIO - This host has been blocked fom hosting this game!💣💣 You filty pirate 🏴‍☠️🦜⛵");
				PopupWrapper.openModal("INVALID HOST","NGIO - This host has been blocked fom hosting this game!💣💣 You filty pirate 🏴‍☠️🦜⛵.");
				setTimeout(() => {
	...
	static #handleNewComponentResult(result)
	{
		...
		if (!result.success && result.error.code !== 104 && result.error.code !== 110)
		{
			...
			if (result.error.code==111) 
			{
				...
				window.dispatchEvent(new CustomEvent("c3plugin-session-cancelled",serialized));
			}
		}
...
/** Start NewgroundsIO.objects.Session **/
	...
	class Session extends NewgroundsIO.BaseObject {
         ...
         * @param {function} closeCallback Callback function when the window closes
         */
		openLoginPage(type)
		{
			...
			this.mode = "check";
			if (type=="popup") {//TristanMX ENH
				NGIO.ngpopupwindow=window.open(this.passport_url, "_blank","height=480,width=480,status=no,toolbar=no,menubar=no,location=no,resizable=no"); //TristanMX ENH
			}
			else {
				NGIO.ngpopupwindow=window.open(this.passport_url, "_blank"); //TristanMX ENH
			}
		}
